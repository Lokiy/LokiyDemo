...
public class XImageView extends ImageView implements ImageLoadingListener, View.OnClickListener, ImageLoadingProgressListener {
	...
	public XImageView(Context context, AttributeSet attrs, int defStyle) {
		...
		mDefaultImage = a.getResourceId(R.styleable.XImageView_defaultImg, android.R.color.transparent);

		mType = a.getInt(R.styleable.XImageView_roundType, DEFAULT_TYPE);
	...
	public static synchronized void initImageLoader(Context context) {
		DisplayImageOptions defaultDisplayImageOptions = new DisplayImageOptions.Builder().bitmapConfig(Config.RGB_565).cacheOnDisk(true).cacheInMemory(true).build();
		Builder builder = new ImageLoaderConfiguration.Builder(context).threadPriority(Thread.NORM_PRIORITY - 2).denyCacheImageMultipleSizesInMemory().tasksProcessingOrder(QueueProcessingType.LIFO).threadPoolSize(5).defaultDisplayImageOptions(defaultDisplayImageOptions).memoryCacheSizePercentage(60);
...
//		}
	...
	private void setDefaultDrawable() {
		if (!isEmptyShow) {
			int res = mDefaultImage == 0 ? R.mipmap.ic_launcher : mDefaultImage;
